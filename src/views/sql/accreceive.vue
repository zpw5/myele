...
export default {
  ...
  methods: {
    ...
    get_org() {
      var sql =
        "select distinct org_id as id,org_name as label from user_org_dept where user_id=" +
      ...
      sqlquery(params).then((response) => {
        ...
        this.partoptions[0].children = response.data.data;
        this.bt4_click();
      });
    },
    //为部门树赋值
    ...
    bt4_click() {
      ...
      if (tosqlin(nodechecked) == "0") {
        sql =sql +
          "select series_id,t_xAxis,t_value from vvt_data where org_id in (" +
          tosqlin(getcol(this.partoptions[0].children, "id")) +
          ") and  search_option='" + y_m_d_val + 
          "' and kpi_name='" + this.agg_type_value +
          "' and element_id='accreceive'";
      } else {
        sql =sql +
          "select series_id,t_xAxis,t_value from vvt_data where " +
          "org_id in (" + tosqlin(nodechecked) +
          ") and  search_option='" + y_m_d_val +
          "' and kpi_name='" + this.agg_type_value +
          "' and element_id='accreceive'";
      }
